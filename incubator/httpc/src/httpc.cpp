...
namespace httpc {
    ...
    // 默认 User-Agent.
    inline const std::string default_user_agent =
        "httpc/1.0 (Boost.Beast)";

    // 常用 User-Agent 常量.
    inline const std::string chrome_user_agent =
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/120.0.0.0 Safari/537.36";

    inline const std::string firefox_user_agent =
    ...
    net::awaitable<boost::system::error_code> http_client::async_connect(
        const urls::url_view& url)
    {
        ...
        auto resolve_result = co_await resolver.async_resolve(
        ...
        if (use_ssl)
        {
            // ---- HTTPS ----
            // 1. 创建并连接 TCP 流
            auto tcp_sock = std::make_unique<tcp_stream>(
                co_await net::this_coro::executor);
            tcp_sock->expires_after(connect_timeout_);
            co_await tcp_sock->async_connect(resolve_result,
                net::redirect_error(ec));
            if (ec)
                co_return ec;

            // 2. 构造 SSL 流 (接管已连接的 TCP 流)
            auto ssl_sock = std::make_unique<ssl_stream>(
                std::move(*tcp_sock), ssl_ctx_);
            // tcp_sock 此时为空 (moved-from)

            // 3. SSL 握手
            ssl_sock->next_layer().expires_after(connect_timeout_);
            co_await ssl_sock->async_handshake(ssl::stream_base::client,
                net::redirect_error(ec));
            if (ec)
                co_return ec;
        ...
        const urls::url_view& url, const http_request& req)
    {
        ...
        {
            boost::system::error_code ec;
            co_await http::async_write(*stream_ptr, request,
                net::redirect_error(ec));
            co_return ec;
        };
    ...
    net::awaitable<http_result> http_client::async_read_response()
    {
        ...
        {
            boost::system::error_code ec;
            co_await http::async_read_header(*stream_ptr, buffer_, parser,
                net::redirect_error(ec));
            co_return ec;
        };
        ...
        if (!parser.is_done())
        {
            ...
            while (!parser.is_done())
            {
                ...
                {
                    boost::system::error_code ec;
                    co_await http::async_read_some(*stream_ptr, buffer_, parser,
                        net::redirect_error(ec));
                    co_return ec;
                };
