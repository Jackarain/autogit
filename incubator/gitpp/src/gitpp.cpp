...
{
    const auto* raw = git_blob_rawcontent(reinterpret_cast<const git_blob*>(obj_.get()));
    return std::span<const std::byte>(static_cast<const std::byte*>(raw), size());
}

std::string_view blob::text() const noexcept
{
    const auto* raw = git_blob_rawcontent(reinterpret_cast<const git_blob*>(obj_.get()));
    return raw ? std::string_view(static_cast<const char*>(raw), size()) : std::string_view {};
}

std::size_t blob::size() const noexcept
{
    return git_blob_rawsize(reinterpret_cast<const git_blob*>(obj_.get()));
}
...
{
    const auto* raw = git_commit_message(reinterpret_cast<const git_commit*>(obj_.get()));
    return raw ? std::string_view(raw) : std::string_view {};
}

oid commit::tree_id() const
{
    return oid(git_commit_tree_id(reinterpret_cast<const git_commit*>(obj_.get())));
}

git_time_t commit::time() const noexcept
{
    return git_commit_time(reinterpret_cast<const git_commit*>(obj_.get()));
}

std::string commit::author_name() const
{
    const auto* sig = git_commit_author(reinterpret_cast<const git_commit*>(obj_.get()));
    return sig ? std::string(sig->name) : std::string {};
}

std::string commit::committer_name() const
{
    const auto* sig = git_commit_committer(reinterpret_cast<const git_commit*>(obj_.get()));
    return sig ? std::string(sig->name) : std::string {};
}
...
tree_entry::tree_entry(const git_tree_entry* entry) noexcept
    : entry_(const_cast<git_tree_entry*>(entry))
    , owned_(false)
{
}
...
{
    return entry_ ? std::string_view(git_tree_entry_name(entry_)) : std::string_view {};
}
...
tree_entry tree::by_path(std::string_view path) const
{
    ...
    exception::throw_on_error(
        git_tree_entry_bypath(&entry, native_tree(), std::string(path).c_str()));
...
void remote::fetch(
    const git_strarray* refspecs, const git_fetch_options* opts, const std::string& reflog_message)
{
    if (!rm_)
        throw std::logic_error("remote::fetch: null remote");
    exception::throw_on_error(git_remote_fetch(rm_.get(),
        refspecs,
        opts,
...
{
    return ref_ ? std::string_view(git_reference_shorthand(ref_.get())) : std::string_view {};
}
...
repo::repo(const std::filesystem::path& repo_dir)
{
    ...
    int err = git_repository_open_ext(&r,
    ...
    const commit& parent)
{
    ...
    exception::throw_on_error(git_commit_create(&commit_id,
        repo_.get(),
...
diff repo::diff_trees(const tree& old_tree, const tree& new_tree)
{
    check();
    git_diff* d = nullptr;
    exception::throw_on_error(git_diff_tree_to_tree(&d,
        repo_.get(),
...
diff repo::diff_tree_to_workdir(const tree& old_tree)
{
    check();
    git_diff* d = nullptr;
    exception::throw_on_error(git_diff_tree_to_workdir(&d,
        repo_.get(),
        const_cast<git_tree*>(old_tree.native_tree()),
        nullptr));
    return diff(d);
...
diff repo::diff_index_to_workdir()
{
    check();
    git_diff* d = nullptr;
    exception::throw_on_error(git_diff_index_to_workdir(&d, repo_.get(), nullptr, nullptr));
    return diff(d);
}
...
bool is_git_repo(const std::filesystem::path& dir)
{
    git_repository* r = nullptr;
    int err =
        git_repository_open_ext(&r, dir.string().c_str(), GIT_REPOSITORY_OPEN_NO_SEARCH, nullptr);
    if (err == 0)
        git_repository_free(r);
    return err == 0;
}

repo init_repo(const std::filesystem::path& repo_path, const std::string& url, bool bare)
{
