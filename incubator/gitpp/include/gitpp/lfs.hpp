...
namespace lfs {
    ...
    constexpr std::string_view k_pointer_version = "https://git-lfs.github.com/spec/v1";
    ...
    struct pointer
    {
        ...
        GITPP_NODISCARD static std::optional<pointer> decode(std::string_view text) noexcept;
        GITPP_NODISCARD static std::optional<pointer> create_from_file(
            const std::filesystem::path& file_path, const std::filesystem::path& gitdir);
        GITPP_NODISCARD static bool is_pointer(std::string_view text) noexcept;
        GITPP_NODISCARD static bool is_pointer_file(const std::filesystem::path& path);

        // 将指针写入文件
        GITPP_NODISCARD bool write_to(const std::filesystem::path& dest) const;
        // 从文件读取指针
        GITPP_NODISCARD static std::optional<pointer> read_from(const std::filesystem::path& path);
    };
    ...
    public:
        ...
        GITPP_NODISCARD std::optional<pointer> store(const std::filesystem::path& file_path);
        ...
        GITPP_NODISCARD std::optional<pointer> store_temp(const std::filesystem::path& tmp_path);

        GITPP_NODISCARD std::filesystem::path object_path(const std::string& oid) const;
    ...
    GITPP_NODISCARD std::vector<std::string> load_lfs_patterns(const std::filesystem::path& gitdir,
        git_repository* repo);

    GITPP_NODISCARD bool path_matches_lfs(std::string_view path,
    ...
    GITPP_NODISCARD int register_lfs_filter(const std::filesystem::path& gitdir);
